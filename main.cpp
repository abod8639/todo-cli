...
int main() {
  ...
  json::DayMap      days_data = json::load(DATA_FILE);
  ...
  auto raw_input = Input(&new_task, " Add task and press Enter...");

  auto root = CatchEvent(
    Renderer(raw_input, [&] {
      int total = (int)tasks.size();
      int done  = 0;
      for (auto &t : tasks) if (t.is_done) done++;
      float progress = total > 0 ? (float)done / total : 0.f;
      std::string pct = std::to_string(done) + "/" + std::to_string(total) +
      ...
      auto task_box = task_area | frame | size(HEIGHT, GREATER_THAN, 6) |
                      borderStyled(ROUNDED,
      ...
      if (focus == AppFocus::INPUT) {
        ...
        hint = hbox({
          ...
          text("  up/down jk:move   Space:toggle   D:delete   Esc/Tab:back   Q:quit") | dim,
        });
      }
      ...
      return vbox({
        ...
        hbox({
          ...
          gauge(progress) | color(
            progress < 0.3f ? Color::RGB(200,60,60) :
            progress < 0.7f ? Color::Yellow :
            Color::RGB(57,211,83)) | flex,
          text(" " + pct + " ") | color(Color::GrayLight),
        }),
        separatorEmpty(),
        input_row,
        separatorEmpty(),
    ...
    [&](Event e) -> bool {
      ...
      if (focus == AppFocus::INPUT) {
        ...
        if ((e == Event::Tab || e == Event::ArrowDown) && !tasks.empty()) {
